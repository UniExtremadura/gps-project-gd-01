...
class ShowDetailsComicActivityTest {
    ...
    @Test
    fun showDetailsComicActivityTest() {
        ...
        textView.check(matches(withText("COMICS")))
        ...
        textView2.check(matches(withText("Ant-Man (2003) #1")))

        textView2.perform(click())

        val textView3 = onView(
            allOf(
                withId(R.id.valorISBN), withText("No existe en la base de datos"),
                withParent(withParent(withId(R.id.nav_host_fragment_activity_main))),
                isDisplayed()
            )
        )
        textView3.check(matches(withText("No existe en la base de datos")))

        val textView4 = onView(
            allOf(
                withText("Ant-Man (2003) #1"),
                withParent(
                    allOf(
                        withId(R.id.toolbar),
        ...
        textView4.check(matches(withText("Ant-Man (2003) #1")))
    }
